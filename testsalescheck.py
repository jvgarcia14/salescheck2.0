...
#
#   ✅ /registergoal <team>      (run inside a topic to save thread_id)
#   ✅ /registergoalall          (run inside a topic; posts ALL teams there)
#   ✅ /goalboard                (manual goalboard for current team group)
#   ✅ /lastsales                (DEBUG: show last saved sales for this team)
#   ✅ FIX: scheduled job callback is properly awaited (no silent cron fail)
#   ✅ FIX: no duplicate pages via canonicalization
#   ✅ FIX: huge table reduced by team_pages + shift-sales union
# ==========================================
...
# ----------------- CONFIG -----------------
BOT_VERSION = "2026-01-22-GoalboardFix-v2"
OWNER_ID = 5513230302
...
def db_last_sales(team: str, limit: int = 5):
    with db.cursor() as cur:
        cur.execute(
            "SELECT page, amount, ts FROM sales WHERE team=%s ORDER BY ts DESC LIMIT %s",
            (team, limit)
        )
        return [(canonical_page(str(p)) or str(p), float(a), str(ts)) for (p, a, ts) in cur.fetchall()]
...
async def registergoal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await require_owner(update):
        return
    if not context.args:
        return await update.message.reply_text("Format: /registergoal 1 (run inside the TOPIC you want)")
...
async def clearteampages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"🧹 Cleared saved pages (team_pages) for {team}.\nIt will rebuild as new tags are used."
    )
...
def build_goalboard_text(team: str, when: datetime) -> str:
    ...
    used_pages = set(db_get_team_pages(team))
...
# ----------------- DEBUG -----------------
async def lastsales(update: Update, context: ContextTypes.DEFAULT_TYPE):
    team = await require_team(update)
    if team is None:
        return
    rows = db_last_sales(team, 5)
    if not rows:
        return await update.message.reply_text("No sales saved yet for this team.")
    msg = "🧾 Last saved sales:\n\n" + "\n".join([f"• {p}: {money(a)} — {t}" for p, a, t in rows])
    await update.message.reply_text(msg)
...
async def scheduled_send_all(application):
    ...
    for team in teams:
        dest = report_groups.get(team) or global_dest
        if not dest:
            continue
...
async def job_runner(ctx):
    await scheduled_send_all(ctx.application)
...
def main():
    ...
    app.add_handler(CommandHandler("lastsales", lastsales))
